...
- **RBAC (Role-Based Access Control)**: Enforced 5 definitive hierarchical roles: `platform_admin`, `university_admin`, `student_organisation`, `class_representative`, and `student`.
...
- **Implemented API Modules**:
  ...
  6. `/api/attendance`: Session generation and Session-based self-marking bounds with optional suspicious tracking triggers.
  7. `/api/announcements`: Global / Scoped announcement distribution.
  8. `/api/gamification`: XP mapping and streaks generation. (Leaderboards are explicitly not included in v1).
...
3. **Entity Views**: Building the UI for listing Tasks, rendering XP / Streaks summaries, posting Announcements, checking Attendance, etc.
